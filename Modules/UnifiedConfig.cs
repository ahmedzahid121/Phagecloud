...
namespace PhageVirus.Modules
{
    ...
    public class UnifiedConfig
    {
        ...
        public string Mode { get; set; } = "cloud"; // local, hybrid, cloud - Changed to cloud for lightweight operation
        ...
        {
            // Memory limits (MB) - Reduced for VM stability
            public int MaxMemoryUsage { get; set; } = 50; // Reduced from 200
            public int TargetMemoryUsage { get; set; } = 25; // Reduced from 100
            
            // CPU limits (%) - Reduced for VM stability
            public int MaxCpuUsage { get; set; } = 3; // Reduced from 10
            public int TargetCpuUsage { get; set; } = 1; // Reduced from 5
            
            // Scan intervals (seconds) - Increased to reduce resource usage
            public int ProcessScanInterval { get; set; } = 300; // Increased from 60
            public int MemoryScanInterval { get; set; } = 600; // Increased from 120
            public int FileScanInterval { get; set; } = 900; // Increased from 300
            
            // Batch processing - Reduced batch sizes
            public int MaxBatchSize { get; set; } = 10; // Reduced from 50
            public int TelemetryBatchSize { get; set; } = 5; // Reduced from 25
        ...
        {
            // Ultra-lightweight mode - minimal local processing for VM stability
            Performance.MaxMemoryUsage = 50; // Reduced from 100
            Performance.MaxCpuUsage = 3; // Reduced from 5
            Performance.ProcessScanInterval = 300; // Increased from 120
            Performance.MemoryScanInterval = 600; // Increased from 300
            Performance.FileScanInterval = 900; // Increased from 600
            
            // Disable ALL heavy local modules, rely on cloud
            ...
            Modules.VirusHunter = false; // Cloud-based - This was causing high memory usage
            Modules.SystemHacker = false; // Cloud-based - This was causing high memory usage
            Modules.SelfReplicator = false; // Disabled for VM stability
            Modules.PhageSync = false; // Disabled for VM stability
            Modules.HoneyProcess = false; // Disabled for VM stability
            Modules.RollbackEngine = false; // Disabled for VM stability
            
            // Keep only essential lightweight modules
            Modules.ProcessWatcher = true; // Lightweight
            Modules.MemoryTrap = true; // Lightweight
            Modules.CredentialTrap = true; // Lightweight
            Modules.ExploitShield = true; // Lightweight
            Modules.FirewallGuard = true; // Lightweight
            Modules.WatchdogCore = true; // Lightweight
            Modules.AutorunBlocker = true; // Lightweight
            Modules.SandboxMode = true; // Lightweight
            Modules.PhishingGuard = true; // Lightweight
            Modules.DnsSinkhole = true; // Lightweight
            Modules.ZeroTrustRuntime = true; // Lightweight
            Modules.PayloadReplacer = true; // Lightweight
            
            // Enable cloud features
            Cloud.Enabled = true;
            
            EnhancedLogger.LogInfo("Applied ultra-lightweight cloud mode optimizations - minimal local processing for VM stability");
        }
