        ...
        private static void MonitorMemoryRegions()
        {
            while (isMonitoring)
            {
                try
                {
                    ...
                    if (DateTime.Now - lastScanTime < scanThrottle)
                    {
                        Thread.Sleep(10000); // Increased sleep interval
                        continue;
                    }
                    ...
                    var targetProcesses = processes.Where(p => 
                        ...
                        ShouldScanProcess(p)
                    ).Take(2); // Reduced from 3 to 2 processes per scan cycle
                    ...
                    // Clean up scanned processes list periodically
                    if (scannedProcesses.Count > 20) // Reduced from 50
                    {
                    ...
                    Thread.Sleep(60000); // Check every 60 seconds instead of 30
                }
                catch (Exception ex)
                {
                    EnhancedLogger.LogWarning($"Memory monitoring error: {ex.Message}");
                    Thread.Sleep(120000); // Longer sleep on error
                }
            }
        }
